...
| total_crawled_resources          | INTEGER |     -      | Total count of HTTP Archive crawl's request & response pairs (counts domain, subdomain and specific page crawls for every crawled domain)                                                                                                                                                              |
| total_crawled_domains            | INTEGER |     -      | Total count of HTTP Archive crawl's request & response pairs (counts only unique domains (base domain names), applying condition `WHERE firstReq = true`)                                                                                                                                              |
...
| total_crawled_resources_with_nel | INTEGER |     -      | Total count of responses that returned with NEL header contained in it's headers (counts every occurrence in monthly data, but the returned data itself contains only those responses that also contained Report-To headers with NEL.report-to header value matching the Report-To.group header value) |
